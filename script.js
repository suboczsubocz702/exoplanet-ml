...
function renderResult(probability, prediction, isFallback = false) {
    if (!resultArea) return;
    
    const isExoplanet = prediction === 1 || probability > 0.55;
    const percent = (probability * 100).toFixed(1);
    const gradient = isExoplanet 
    ...
    resultArea.innerHTML = `
        ...
        <div style="text-align: center;">
            ...
            <div style="margin: 1rem 0;">
                <div style="font-size: 0.75rem; margin-bottom: 0.25rem;">Confidence</div>
                <div style="background: rgba(0,0,0,0.4); border-radius: 1rem; height: 8px; overflow: hidden;">
                    <div style="background: ${gradient}; width: ${percent}%; height: 100%;"></div>
                </div>
                <div style="font-size: 1.2rem; font-weight: 600; margin-top: 0.5rem;">${percent}%</div>
            </div>
            <div style="font-size: 0.75rem; opacity: 0.7; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 0.75rem;">
